...
import TaxDocumentViewer from '@/components/TaxDocumentViewer';
import { useRouter } from 'next/navigation';

const validUsTaxTypes = [
    'application/pdf',
    'image/png',
    'image/jpeg',
    'image/jpg',
...
const UploadStatusIndicator = ({
    ...
    return (
        <div className='fixed bottom-8 right-8 z-50 w-96 bg-[#252550] rounded-lg shadow-lg p-4 border border-indigo-400'>
            <h4 className='text-white mb-2 font-bold text-lg'>
                US Tax Paper Uploads
            </h4>
            <ul>
...
const UsTaxPage = () => {
    ...
    const handleCreateFolder = async (folderName: string) => {
        try {
            if (!user?.customerNumber)
                throw new Error('User customer number required');
    ...
    const handleDeleteFile = async (fileId: number, customerId: string) => {
        try {
            await api.delete(`/file`, {
                params: { fileId: fileId, customerId: customerId },
                withCredentials: true,
            });
            ...
            setFolders(prev =>
                prev.map(folder => ({
                    ...folder,
                    documents:
                        folder.documents?.filter(doc => doc.id !== fileId) ??
                        [],
                    videos:
                        folder.videos?.filter(vid => vid.id !== fileId) ?? [],
                })),
            );
    ...
    const handleUploadDocument = async (folderId: number, file: File) => {
        ...
        if (!validUsTaxTypes.includes(file.type)) {
            toast.error('Unsupported file type. Only PDF and images allowed.');
            return;
        }
        ...
        try {
            ...
            await new Promise<void>((resolve, reject) => {
                ...
                xhr.onload = () =>
                    xhr.status >= 200 && xhr.status < 300
                        ? resolve()
                        : reject(
                              new Error(`Azure upload failed: ${xhr.status}`),
                          );
            ...
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_REDAXIFY_BACKEND_URL}/api/document/upload`,
                {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
            ...
            toast.success('US tax paper uploaded successfully!');
        } catch (error) {
            console.error('Upload failed:', error);
            setUploadStates(prev => ({
                ...prev,
                [folder.id]: { ...prev[folder.id], status: 'error' },
            }));
            toast.error('US tax paper upload failed');
        }
    };
                    ...
                    <div className='absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-[#1C1C3A]/90 to-[#242648]/90 p-8'>
                        <div className='text-center max-w-md'>
                            ...
                            <h3 className='text-2xl font-bold text-white mb-2'>
                                No US tax paper Selected
                            </h3>
                            <p className='text-gray-300 mb-6'>
                                Upload a new US tax paper or select one from
                                your folders.
                            </p>
                ...
                {isFoldersLoading
                    ...
                    : folders.map(folder => (
                          <DocumentFolderComponent
                              ...
                              onUpload={file =>
                                  handleUploadDocument(folder.id, file)
                              }
